...
def load_template(relative_path: str, fallback: str) -> str:
    candidate = PROMPTS_DIR / relative_path
    if candidate.exists():
...
class LLMClient:
    ...
    def draft_requirements(self, ctx: WorkspaceContext) -> str:
        ...
        system_prompt = (
            "You are Code Machine. Given a user prompt, draft Markdown requirements "
            "with sections: Overview, Functional Requirements, Data, Non-Functional, Acceptance."
        )
    ...
    def draft_architecture(self, ctx: WorkspaceContext, requirements: str) -> str:
        ...
        template = load_template(
            "plan/plan_arch.txt",
            "You are a software architect. Given requirements, produce Markdown architecture with components,"
            " tech stack, and diagrams.",
        )
        messages = [
    ...
    def extract_tasks(self, ctx: WorkspaceContext, plan_markdown: str) -> List[Dict[str, Any]]:
                    ...
                    "tasks": [
                        {
                            ...
                            "input_files": [],
                            "target_files": [f"{ARTIFACTS_DIR}/{REQUIREMENTS_FILE}"],
                            "dependencies": [],
                            "acceptance": ["Requirements captured"],
                        },
                        {
                            "task_id": "I1.T2",
                            "description": "Write architecture.md",
                            "input_files": [f"{ARTIFACTS_DIR}/{REQUIREMENTS_FILE}"],
                            "target_files": [f"{ARTIFACTS_DIR}/{ARCHITECTURE_FILE}"],
                            "dependencies": ["I1.T1"],
                            "acceptance": ["Architecture outlines components"],
                        },
                    ],
                },
                {
                    ...
                    "tasks": [
                        {
                            ...
                            "input_files": [
                                f"{ARTIFACTS_DIR}/{REQUIREMENTS_FILE}",
                                f"{ARTIFACTS_DIR}/{ARCHITECTURE_FILE}",
                            ],
                            "target_files": [
                            ...
                            "dependencies": ["I1.T2"],
                            "acceptance": ["Plan stored", "todo.json structured"],
                        }
                    ],
                },
            ]
        ...
        system_prompt = (
            "You are a task extractor. Convert the iteration plan into JSON with fields: "
            "iteration_id, task_id, description, input_files, target_files, dependencies, acceptance."
            "Return a JSON array of iterations, each containing a tasks array."
        )
        messages = [
            {"role": "system", "content": system_prompt},
...
def command_project(args: argparse.Namespace, llm: LLMClient) -> None:
    ...
    pipeline = TypeAPipeline(ctx, llm)
    todo = pipeline.generate_documents(force=args.force)
